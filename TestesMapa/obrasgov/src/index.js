...
async function main() {
  ...
  while (true) {
    const response = await apiService.fetchObrasPorUF(
      uf,
      pagina,
      tamanhoDaPagina,
    );
    ...
    if (pagina === 0 && response.totalElements) {
      ...
      console.log(
        `Número total de obras (relatado pela API): ${totalElements}`,
      );
    }
    ...
    if (obras.length === 0) {
      ...
      console.log(
        `Página ${pagina} vazia. (${paginasVaziasConsecutivas}/${limitePaginasVazias} páginas vazias consecutivas)`,
      );
  ...
  await fileService.saveToJsonFile(
    obrasComGeometria,
    'obras_com_lat_long.json',
  );
  ...
  console.log(
    `Processamento concluído. Total de obras processadas: ${obrasComGeometria.length}`,
  );
}
