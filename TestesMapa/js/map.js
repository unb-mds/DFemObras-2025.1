...
function inicializaMapa() {
  const bounds = L.latLngBounds(L.latLng(-16.1, -48.2), L.latLng(-15.5, -47.5));
...
function determinarRA(latitude, longitude) {
  ...
  } else if (lat >= -15.9 && lat <= -15.75 && lon >= -48.2 && lon <= -47.85) {
    return 'Região Oeste do DF';
  } else if (lat >= -16.1 && lat <= -15.85 && lon >= -48.2 && lon <= -47.85) {
    ...
    .then((dados) => {
      return dados.map((obra, index) => {
        if (obra.latitude && obra.longitude) {
          obra.regiao_administrativa = determinarRA(
            obra.latitude,
            obra.longitude,
          );
        } else {
...
function processarDadosDasObras(dados, mapa) {
  ...
  dados.forEach((obra, indice) => {
    ...
    if (!latitude || !longitude) {
      console.log(
        `Obra ${indice}: "${nome}" ignorada por falta de coordenadas.`,
      );
      return;
    }
    ...
    const conteudoPopup = gerarConteudoDoPopup(
      nome,
      situacao,
      formatarBRL(valor),
      indice,
...
function popularDropdownSituacao() {
  ...
  const situacoes = [
    ...new Set(todasAsObras.map((obra) => obra.situacao).filter(Boolean)),
  ];
...
function popularDropdownEspecie() {
  ...
  const especies = [
    ...new Set(todasAsObras.map((obra) => obra.especie).filter(Boolean)),
  ];
...
function aplicarFiltros() {
  ...
  filteredObras = todasAsObras.filter((obra) => {
    ...
    return (
      matchRA && matchSituacao && matchEspecie && matchSearch && matchAtraso
    );
  });
  ...
  updateFilterStatus(
    `${filteredObras.length} obras exibidas (${atrasadas} atrasadas)`,
  );
}
...
function getStatusClass(situacao) {
  ...
  if (status.includes('execução') || status.includes('execucao'))
